...
/**
 * Shopping cart controller call at Addweb_AdvReservation_Checkout_CartController
 */
require_once "Mage/Checkout/controllers/CartController.php";  
class Addweb_AdvReservation_Checkout_CartController extends Mage_Checkout_CartController
{
	 ...
	 * here is frontend shoppinfg cart update code customization in this "_updateShoppingCart()".
	 * custom create array for $cartData and get $getSession for shopping cart create same array for $cartData.
	 * add if condition for validation calculated Qty value can't update customer increase but can't decrease calculated value.
     */
    ...
    { 
        try {
			...
			$customArrData = array();
	 		foreach	($cartData as $cartDataId => $cartDataQty)	{
				$customArrData[] =  $cartDataQty;
			}	
			
			foreach($customArrData as $customArrId => $customArrQty){
				if($customArrData[$customArrId]['qty'] >= $getSession[$customArrId]['qty']){
				}
				else{
					$message = "You cannot decrease the calculated quantity!";
					Mage::getSingleton('core/session')->addError($message);
					return $getSession[$customArrId][$customArrQty['qty']];
				}
    ...
    public function updateItemOptionsAction()
    {
        ...
        try {
            ...
            if (!$this->_getSession()->getNoCartRedirect(true)) {
                if (!$cart->getQuote()->getHasError() && false) {
